...
class BrowserAgent:
        ...
        async with PLAYWRIGHT_LOCK:
            ...
            try:
                ...
                self._pw_context = await self._pw.chromium.launch_persistent_context(
                    ...
                    headless=False,  # ✅ force visible (like Planner/Buying)
                    args=[
                        "--no-sandbox",
    ...
    async def _multi_vision_summarize(self, query: str, site_results: list[dict]) -> str:
        ...
        system_prompt = (
            "You are Mira, a calm and factual research assistant.\n"
            "- Combine key facts concisely.\n"
            "- Emphasize verified details on sports scores, headlines, or summaries.\n"
            "- Ignore ads, popups, or redundant text.\n"
            "- Write in natural language, no markdown.\n"
            "- Mention publication name casually when clear (e.g., 'ESPN reports...')."
        )

        messages = [
            SystemMessage(content=system_prompt),
            HumanMessage(content=f"User asked: {query}\nReview all extracts and screenshots for key info.")
        ]
                ...
                if ss_path and os.path.exists(ss_path):
                    ...
                    messages.append(
                        HumanMessage(content=[
                            {"type": "text", "text": f"Screenshot from {s['url']} — extract visible facts or scores."},
                            {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{img_b64}"}}
                        ])
                    )
                messages.append(HumanMessage(content=f"Extracted from {s['url']}:\n{clean_text}"))
            except Exception as e:
        ...
        try:
            ...
            return "I couldn’t summarize this right now."

    # ======================================================
    # 🔹 Multi-site orchestration + summarization
    # ======================================================
    async def multi_site_answer(self, query: str, urls: List[str]) -> str:
        """
        Crawl multiple URLs concurrently, summarize them via GPT-4o multimodal,
        and clean up screenshots after summarization.
        """
        ...
        for s in site_results:
            try:
                ...
                logger.log_error(e, context="BrowserAgent.multi_site_answer.cleanup")
