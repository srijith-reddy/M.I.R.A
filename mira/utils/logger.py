...
def log_info(msg: str, context: str = ""):
    """
    Logs info-level messages consistently with timestamp.
    """
    entry = f"[{_timestamp()}] INFO"
    if context:
        entry += f" [{context}]"
    entry += f": {msg}"
    print(entry)
    with open(os.path.join(LOG_DIR, "info.log"), "a") as f:
        f.write(entry + "\n")
